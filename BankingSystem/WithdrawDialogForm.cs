...
{
    public partial class WithdrawDialogForm : Form
    {
        int accountId;
        private readonly BankingSystemDBEntities context;
        public WithdrawDialogForm(int Id)
        {
            InitializeComponent();
        ...
        public bool CreateTransaction(string type, DateTime date, int employeeId, int accountId, decimal amount)
        {
            ...
            catch (SqlException ex)
            {
                ...
                {
                    MessageBox.Show("An error occurred while processing the transaction.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                return false; 
            }
        }
